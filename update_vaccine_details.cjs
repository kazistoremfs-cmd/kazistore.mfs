...
let updated = content.replace(
  "import { ArrowLeft, CheckCircle2, Syringe, Info, ChevronRight } from 'lucide-react';",
...
// 3. Update cholera data
const oldCholeraStart = "    cholera: {";
const oldCholeraRegex = /cholera:\s*\{[\s\S]*?\}\s*\};/;
const newCholera = `cholera: {
...
updated = updated.replace(oldCholeraRegex, newCholera);

// 4. Add getTotalFee method
updated = updated.replace(
...
// 5. Update Header to add Symptoms Collapse
const headerRegex = /<div className="bg-slate-800\/60 border border-slate-700\/50 rounded-2xl p-6 md:p-8 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-start md:items-center gap-6">[\s\S]*?<\/div>\s*<\/div>/;
const newHeader = `<div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6 md:p-8 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-start md:items-center gap-6">
...
updated = updated.replace(headerRegex, newHeader);

// 6. Conditionally render Process
const processRegex = /<div className="grid grid-cols-1 md:grid-cols-2 gap-6">[\s\S]*?{currentData\.process\.map\(\(step, index\) => \([\s\S]*?<\/ul>\s*<\/div>/;
const newProcess = `<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
...
updated = updated.replace(processRegex, newProcess);

// Handle col-span if process is hidden
updated = updated.replace(
...
// 7. Update fee section
const feeRegex = /<div className="bg-blue-900\/20 border border-blue-500\/20 rounded-xl p-4">[\s\S]*?<\/div>\s*<\/div>/;
const newFee = `<div className="bg-blue-900/20 border border-blue-500/20 rounded-xl p-4 mt-6">
                ...
                <div className="mt-3 text-xl md:text-2xl font-bold text-emerald-400 font-bn text-center bg-slate-900/50 py-2 rounded-lg border border-slate-700 flex justify-center items-center gap-2">
                  <span className="text-lg md:text-xl text-slate-300">মোট সার্ভিস চার্জ:</span>
                  <span>{getTotalFee()} টাকা</span>
                </div>
              </div>
            </div>`;
updated = updated.replace(feeRegex, newFee);
